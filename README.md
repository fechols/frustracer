...
| `FR_WAVEVIZ=1` \| `chs` | Wave-footprint visualization: every wave takes a ticket, every pixel wears its wave's hash color — **I** toggles it live in GPU sessions; headless `--spin` runs dump `waveviz-<arm>.png` + compactness stats. `chs` tickets the mode-1 closest-hit instead (shows the driver's hit-stage packing) |
