...
cargo run --release -- --gpu --continuation-rays  # A/B measurement lever (default OFF;
                                        ...
                                        # now proven from the other side). Known-accepts v1:
...
cargo run --release -- --lock-res 0.75     # lock the render res to a fixed scale of the window; the
                                           # default is `quality` (2/3, xess::DEFAULT_LOCK_SCALE) for
                                           # EVERY render mode — CPU, --gpu and --dxr alike, so F/SPACE
                                           # cycling arms never moves the render res. (Until 2026-07-26
                                           # the GPU arms defaulted to native 100% through a second
                                           # `Opts::gpu_lock_scale`; that field and the split are gone,
                                           # so GPU-mode numbers taken before it are at 2.25x the
                                           # PIXELS a flagless session now traces — 2/3 is a LINEAR
                                           # scale, 1920x1080 -> 1280x720, so pixel-proportional costs
                                           # scale by 0.444 and per-tile ones (the level ladder) barely
                                           # move at all. The one carve-out is
                                           # headless `--spin`, which stays at native unless --lock-res
                                           # is passed — benchmarks must not have defaults move under
                                           # them, the vendor-default rule. Two accepts ride the
                                           # retirement, both spelled out in the GPU-born-G-buffers
                                           # paragraph: toggling the upscaler OFF mid-session (G/X/K)
                                           # presents the LOCKED res stretched, so "plain" is 2/3 there
                                           # and native under --no-upscale; and vendor_defaults' Intel
                                           # entry was measured at native, an OPEN DEBT its own comment
                                           # now carries.) Also takes native (100%)
...
cargo run --release -- --gpu --xess   # GPU tracer -> XeSS-SR composition (implies --no-dlss); the
                                      ...
                                      # 2/3 — `--lock-res dynamic` is not honorable under --gpu, it
...
cargo run --release -- --dxr          # the by-the-book DXR pipeline (RTPSO + SBT + DispatchRays with
                                      ...
                                      # --lock-res scale (default quality 2/3 — the ONE
...
cargo run --release -- --spin path    # headless deterministic benchmark/profiling workload: the
                                      ...
                                      # `--lock-res` scale — NATIVE unless --lock-res is passed, the
                                      # one place the quality default does NOT apply (a benchmark's
                                      # res must not move under recorded numbers) — with the SAME
...
cargo run --release -- --cinematic tour --cinematic-res 3840x2160 --cinematic-fps 60 \
                        --cinematic-frames 1200 --cinematic-hdr
                                      ...
                                      # README/release. Presets hero|islands|tour|orbit|hud|list,
                                      ...
                                      # them). Sub-flags: -res WxH (odd dims round down — yuv420p),
                                      # -samples N (accumulated sub-frames per OUTPUT frame; the
                                      # ONLY quality knob, since no upscaler exists headlessly —
                                      # composes with --spp, a different axis that amortizes the
...
A first-class headless MEDIA mode, peer to `--check` and `--spin`: stills and camera-spline video sequences, rendered deterministically for the README/release. `src/cinematic.rs` is the PURE half (data model, spline, presets, HUD composite, `self_test` — no GPU, no platform API, covered by `--check` on every platform); the drivers live in main.rs beside `run_spin`/`run_spin_gpu`, whose frame contract they mirror. Presets: `hero` (one still; bare `--cinematic` = this, plus the catalogue), `islands` (one per world island), `tour` (THE LAP — closed-loop flight over the ring, the attractors sweeping dawn → moonlit night), `orbit`, `hud`, `list`; anything else is a JSON shot-list path. Sub-flags all carry the `--cinematic-` prefix precisely so ONE `starts_with` clause covers the family in `settings::headless_args` (the `--spin`/`--spin-frames` two-arm wart, avoided).

**WHY IT EXISTS, given `--spin` already walks a camera path**: `--spin` is a benchmark that writes no pixels and whose path amplitudes are benchmark-sized (≤ 0.12 diag). More importantly the INTERACTIVE renderer cannot produce these frames. Every cinematic output frame is a STATIC pose accumulating N sub-frames, which buys two things a live session cannot have: proper multi-sample AA with no temporal upscaler, and the hemisphere bounce integrator (`Quality::fb`), which is still-frames-only by construction. **Cinematic is therefore the only path in the tree that can render a moving camera WITH GI.** No upscaler exists headlessly (`GpuContext::new` needs an HWND — `run_spin_gpu`'s own comment), so `--cinematic-samples` is the single quality knob; for a still that is BETTER than the window shows (converged ground truth, no reconstruction artifacts).
...
**ONE presentation path for all three arms**: CPU, wavefront and DXR all hand a LINEAR f32 image to `cine_write_frame` → `render::resolve_hdr` → `tone::ToneParams::SDR` → `save_png`. Nothing goes through `record_resolve`/`hdr`/the tonemap PS, which would put the GPU arms on the shader's curve and the CPU arm on tone.rs's. The GPU arms read `accum` back ONCE per OUTPUT frame (not per sub-frame). Arm policy differs from `--spin` on purpose: cinematic takes the best available arm by DEFAULT (`--gpu` wavefront, else DXR, else CPU with a loud line) because its job is to finish; `--cinematic-gi` forces the wavefront (DXR has no hemi stage) with a loud line rather than silently dropping the feature.
