...
cargo run --release -- --emissive-lights  # ARM emissive surfaces lighting the scene — DEFAULT
                                      ...
                                      # its ms. THE COMPILED OFF IS ONLY HALF THE DEFAULT SINCE
                                      # 2026-08-08 (the user's fourth-round call): a session
                                      # whose WIRED upscaler is TAA-class — XeSS or FSR 3.1 —
                                      # arms it automatically (main::upscaler_defaults, the
                                      # vendor_defaults sibling keyed on GpuContext::wired()
                                      # rather than the adapter — its first main.rs consumer;
                                      # applied in run_window's post-GpuContext opts shadow with
                                      # an emissive::set_enabled re-store, the set_inline_mode
                                      # shape, BEFORE the scene-load spawn so the derivation
                                      # line reports it). Rationale = the --no-rtgi entry's
                                      # NEE-keep known-accept promoted to policy: a TAA
                                      # neighborhood clamp rejects the RTGI bounce's sparse
                                      # stochastic emissive, so unarmed XeSS/FSR3 sessions lose
                                      # the pools outright, while DLSS-RR/FSR4-RR reconstruct
                                      # them and keep OFF (NEE there buys shadow rays for
                                      # transport the bounce delivers); Quin stays OFF (the fuse
                                      # anchors on a denoising engine — the user's call), Plain
                                      # unchanged. Opts::emissive_lights_explicit is the veto —
                                      # EITHER CLI spelling sets it (presence-not-value, the
                                      # dxr_inline_explicit doctrine, making --no-emissive-lights
                                      # the spelled opt-out in XeSS/FSR3 sessions) and so does a
                                      # saved settings-file value (the dxr_inline precedent, NOT
                                      # the fg one — a saved menu preference must veto the
                                      # policy); the menu's live toggle still overrides at
                                      # runtime. --cinematic applies the same policy once per
                                      # run at its first successful chain wiring
                                      # (run_cinematic_gpu — a media mode that already resolves
                                      # a chain; sound post-load because the enable is LIVE and
                                      # derivation runs unconditionally); --check*/--spin keep
                                      # the parse-time store — gates stay a pure function of the
                                      # command line, so the armed must-fires below still need
                                      # an explicit --emissive-lights. cli/settings self_tests
                                      # pin the veto both ways.
...
cargo run --release -- --no-auto-exposure  # kill lever: the interactive aperture holds at exactly
                                      # 1.0 (src/autoexp.rs + gpu/autoexp.rs +
                                      # shaders/autoexp.hlsl — DEFAULT ON, 2026-08-08).
                                      # Auto-exposure is a DISPLAY-STAGE controller in the bloom
                                      ...
                                      # accum/hdr agreement — save/restore levers, so a
                                      # --no-auto-exposure --check still proves the on arm) and
...
cargo run --release -- --no-rtgi      # kill lever: REAL-TIME GI off — the ambient tier reverts to
                                      ...
                                      # it) — RESOLVED as policy 2026-08-08: XeSS/FSR3 sessions
                                      # now arm --emissive-lights by DEFAULT
                                      # (main::upscaler_defaults, see the --emissive-lights
                                      # entry; --no-emissive-lights / a saved settings value
                                      # opts out), so the manual "arm it there" advice applies
                                      # only to sessions that vetoed.
                                      # CONTRACTS:
                                      ...
                                      # un-denoised residual, the accepted v1 known-accept; the
...
cargo run --release -- --lock-res 0.75     # lock the render res to a fixed scale of the window; the
                                           # default is `quality` (2/3, xess::DEFAULT_LOCK_SCALE —
                                           # the wired upscaler reconstructs the window from 0.444x
                                           # the pixels; `--lock-res native` = the DLAA arm) for
                                           # EVERY render mode — CPU, --gpu and --dxr alike, so F/SPACE
                                           # cycling arms never moves the render res. HISTORY, three
                                           ...
                                           # again since. So numbers recorded at "the flagless
                                           ...
                                           # barely move at all), full-res in the native one.
                                           # `--lock-res quality` still spells the
                                           ...
                                           # them, the vendor-default rule (so a flagless --spin now
                                           # again traces MORE pixels than a flagless session).
                                           # The 2/3-era accepts are BACK: G/X/K toggling the
                                           # upscaler OFF mid-session presents the LOCKED sub-native
                                           # res as a DWM-stretched blit, and vendor_defaults' Intel
                                           # entry is NOT measured at the res a flagless session
                                           # traces (its res-basis caveat is RE-OPENED — the
                                           # RE-OPENED DEBT paragraph there). Also takes native (100%)
...
cargo run --release -- --spin path    # headless deterministic benchmark/profiling workload: the
                                      ...
                                      # interactive default is quality 2/3 again since 2026-08-08,
                                      # so the independence is doing real work again, and the
