fn main() {
    ...
    #[cfg(windows)]
    {
        ...
        // Raw-NGX DLSS-G shim — BUILD-OPTIONAL: the NDA-tier DLSS SDK is not
        // redistributable and never committed, so this compiles only when
        // FRUSTRACER_DLSS_SDK (default: the sibling quinlight-player's
        // vendored copy) points at one. Without it the build proceeds and the
        // Rust side stubs frame generation for DLSS sessions back to the
        // Streamline attempt. Links nvsdk_ngx_d.lib (the /MD import stub
        // bound to the driver's _nvngx.dll — no CRT conflict; the static _s
        // variants are /MT) and stages nvngx_dlssg.dll next to the binary
        // (NGX resolves feature snippets from the exe directory).
        println!("cargo:rustc-check-cfg=cfg(dlssg_ngx)");
        ...
        let dlssg_dll = sdk.join(r"lib\Windows_x86_64\rel\nvngx_dlssg.dll");
        if dlssg_header.exists() && import_lib_dir.join("nvsdk_ngx_d.lib").exists() {
            cc::Build::new()
                .cpp(true)
                .std("c++17")
                .flag("/EHsc")
            ...
            println!("cargo:rustc-cfg=dlssg_ngx");
            // Stage the snippet DLL next to the binary for every profile dir
            ...
            if dlssg_dll.exists() {
                if let Ok(out) = std::env::var("OUT_DIR") {
                    ...
                    if let Some(dir) = profile_dir {
                        let _ = std::fs::copy(&dlssg_dll, dir.join("nvngx_dlssg.dll"));
                    }
                }
            }
        } else {
            println!(
                "cargo:warning=DLSS SDK not found at {} — raw-NGX DLSS-G disabled \
                 (set FRUSTRACER_DLSS_SDK to enable)",
                sdk.display()
            );
        }
    }
