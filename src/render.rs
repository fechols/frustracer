...
/// PRESENTS: FSR reconstructs color as dd⊗kd + ds⊗f0 + residual and never
/// reads `accum` on the CPU-fed path. Under `--spp` the presented color is the
/// N-sample average, which only exists after `shade_pixel`'s loop — hence a
/// separate entry point it can call again with the average (see there). The
/// GPU feed kernel obeys the same rule from the other side: it computes the
/// residual against the averaged `accum`.
fn write_fsr(
    ctx: &FrameCtx,
    x: usize,
    y: usize,
    ...
    let (_, far) = dlss::near_far(ctx.scene.diag);
    if !t.is_finite() {
        // Sky: both signals zero, residual = the color itself (albedos don't
        // matter — 0 * anything), prev_z = far so the depth delta is 0.
        let sig = crate::fsr::Signals { dd: Vec3A::ZERO, ds: Vec3A::ZERO, residual: c };
        f.write(x, y, &sig, far);
        return;
    }
...
fn shade_pixel(
    ...
    // planes (dd⊗kd + ds⊗f0 + residual — `accum` is never uploaded on the
    // CPU-fed path), so the residual `shade_traced` wrote for the probe sample
    // is a remainder against THAT sample's color, not the average this frame
    // presents. Rewrite it against the average, or FSR would put sample 0's
    // image back on screen and --spp would be a costly no-op there. The
    // denoised lobes (dd/ds) stay the probe sample's — the GPU pack captures
    // them the same way, and its feed kernel likewise takes the residual
    // against averaged accum, so both feeds mean exactly the same thing.
    if spp > 1 {
