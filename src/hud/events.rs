...
fn special(k: sdl2::keyboard::Keycode) -> Option<Key> {
...
fn button(b: sdl2::mouse::MouseButton) -> PointerEventButton {
...
pub fn forward(win: &slint::Window, ev: &Event) {
    let dispatched = match ev {
        ...
        Event::MouseWheel { precise_x, precise_y, mouse_x, mouse_y, .. } => {
            win.dispatch_event(WindowEvent::PointerScrolled {
                position: LogicalPosition::new(*mouse_x as f32, *mouse_y as f32),
                delta_x: precise_x * 40.0,
                delta_y: precise_y * 40.0,
            });
            true
        }
