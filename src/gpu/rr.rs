...
impl RrResources {
    ...
    /// row slices) and record the copies + barriers. Inputs live in
    ...
    pub fn tags(&self) -> [sl::SlShimResourceTag; 8] {
        ...
        [
            tag(&self.planes[P_COLOR].tex, sl::BUFFER_SCALING_INPUT_COLOR, npsr),
            tag(&self.output, sl::BUFFER_SCALING_OUTPUT_COLOR, uav),
            tag(&self.planes[P_DEPTH].tex, sl::BUFFER_LINEAR_DEPTH, npsr),
            tag(&self.planes[P_MVEC].tex, sl::BUFFER_MOTION_VECTORS, npsr),
            tag(&self.planes[P_ALBEDO].tex, sl::BUFFER_ALBEDO, npsr),
            tag(&self.planes[P_SPEC_ALBEDO].tex, sl::BUFFER_SPECULAR_ALBEDO, npsr),
            tag(&self.planes[P_NORMAL_ROUGH].tex, sl::BUFFER_NORMAL_ROUGHNESS, npsr),
            tag(&self.planes[P_SPEC_HIT].tex, sl::BUFFER_SPECULAR_HIT_DISTANCE, npsr),
        ]
    }
}
