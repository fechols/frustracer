...
fn main() {
    ...
    while let Some(a) = args.next() {
        match a.as_str() {
            ...
            "--cloud-shadow" => {
                ...
                gpu::trace::set_cloud_shadow(n);
            }
            "--no-sky-lod" => gpu::trace::set_sky_lod(1),
            "--sky-lod" => {
                let k: u32 = args
                    .next()
                    .and_then(|s| s.parse().ok())
    ...
    // parse arms / settings already stored the static; only a DEPARTURE from
    // the shipped default speaks (the blas-split lever-line rule).
